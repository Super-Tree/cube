...
def cubic_rpn_grid_pyfc(lidarPoints, rpnBoxes):
    ...
    if DEBUG:
        pass
        pcd_vispy(lidarPoints,rpnBoxes)
    ...
    for box in rpnBoxes:
        ...
        if DEBUG:
            box_mv = [box[0], box[1] - box[1], box[2] - box[2], box[3] - box[3], cfg.ANCHOR[0], cfg.ANCHOR[1],
                      cfg.ANCHOR[2], box[7]]
            pcd_vispy(cubic_feature.reshape(-1, 4), np.array(box_mv),now=False)
...
class cubic(object):
    def __init__(self, batch_size, channel, name=None, training=True):
        ...
        with tf.variable_scope('fc_bn_1', reuse=tf.AUTO_REUSE) as scope:
                ...
                self.bn_fc_1 = tf.layers.BatchNormalization(fused=True, _reuse=tf.AUTO_REUSE, _scope=scope)
    ...
    def apply(self, inputs):
        ...
        out_conv3d_2_pool=self.maxpool_1.apply(out_conv3d_2)
        out_conv3d_3 = self.conv3d_3.apply(out_conv3d_2_pool)
        out_bn_1=self.bn_1.apply(out_conv3d_3)
        out_conv3d_4 = self.conv3d_4.apply(out_bn_1)
        out_bn_2=self.bn_2.apply(out_conv3d_4)

        conv3d_flatten = tf.layers.flatten(out_bn_2)

        dense_out_1 = self.dense_1.apply(conv3d_flatten)
        dense_bn_1 = self.bn_fc_1.apply(dense_out_1)
